...
> **只读这一份 `FOR_CLAUDE.md`，不要打开仓库里的 `FOR_CODEX.md`。**
> `FOR_CODEX.md` 是交给另一个 agent 的**平行任务书**，除了域名和 Cloudflare zone
> 之外内容几乎完全相同。那份用 `avpn.vip`，这份用 `dongziyu.com`；两个 agent 必须在
> **各自独立的域名 / zone / 分支**上部署，互不干扰。读了另一份只会让你混淆该操作哪个
> zone、该新建哪些 DNS 记录、该写哪个部署文档。**本任务书里所有域名一律是 `dongziyu.com`。**
...
- 命名与标签必须可辨识、便于回收（例如实例名 `hatchway-*`、加 `purpose=hatchway` label），
  并把**预估月成本**写进 `docs/deployment-dongziyu.md`。
...
**本次开发只准动 `dongziyu.com`（zone id `5ac42716cde0613ccf53e37e39eae69d`）。**
该 zone 当前 5 条记录：

```
A     openoj.dongziyu.com                -> 34.145.247.213    proxied=False
CNAME ssh-vagrant-debian-01.dongziyu.com -> ef6cf6db-…-eed990edb9cd.cfargotunnel.com  proxied=True
CNAME ssh-vagrant-ubuntu-01.dongziyu.com -> a613fea6-…-4955d6a83fd3.cfargotunnel.com  proxied=True
AAAA  dongziyu.com                       -> 100::             proxied=True
AAAA  www.dongziyu.com                   -> 100::             proxied=True
```

注意：这个 zone **没有任何遗留的 Hatchway 记录**——`api.dongziyu.com` /
`frps.dongziyu.com` / `*.tunnel.dongziyu.com` 都要你新建，指向本次新建 VM 的静态外部 IP。
`openoj.dongziyu.com` 指向的 `34.145.247.213` 是 katze 的外网 IP，属于无关服务。
**上面这 5 条记录（`openoj`、两条 `ssh-vagrant-*`、根域与 `www` 的 AAAA）一律不得修改、
删除或改变 proxy 状态。** 你只能新增并维护 Hatchway 自己的 `api.` / `frps.` / `*.tunnel.`
三条记录。
...
> 临时域名上**（开发期用 `dongziyu.com`），并让 `hatchway-skills` 作为配套的 Agent Skill
> 与之协同工作。
...
2. **真实部署到 GCP VM 并端到端验证**：本地起一个示例 API/Agent 服务 → 一条命令暴露 →
   从公网真实 curl 到 `https://t-*.tunnel.dongziyu.com`（或你设计的新域名形态）拿到 200 →
...
- **只能操作 `dongziyu.com` 这一个 zone**，其它 zone 一律不碰。
...
2. **线上可用的部署**：`dongziyu.com` 下真实可访问的 Hatchway 服务，以及一份能复现该部署的
   脚本/compose/文档（不含明文密钥）。
3. `docs/deployment-dongziyu.md`：本次实际部署的拓扑、主机、端口、域名、密钥位置（只写位置
...
- 任何会改动 `dongziyu.com` 上非 Hatchway 记录、或影响 katze 已有服务的操作——
  这类操作**一律不许执行**，只记录。
